...
static void Destroy_Thread(struct Kernel_Thread* kthread)
{
    ...
    Free_Page(kthread->stackPage);//�ͷ�ѹջ
    Free_Page(kthread);//�ͷ��߳�
 ...
 * i.e., the run queue is never empty.
 只有系统运行队列中没有其他的可运行进程了，才会调度Idle运行。
 */
...
static void Reaper(ulong_t arg)
{
	...
	*/
	    ...
	    Yield();   /* allow other threads to run? 
	     因为Reaper线程永远不会退出，所以要给其他线程运行的机会，这里没有Yield的话系统就死机了
		*/
 ...
 * Calling this function initiates a context switch.
 置线程的退出码和存活标志，然后调用Tlocal_Exit释放将线程的本地内存。
 */
void Exit(int exitCode)
{
	...
	*/
    ...
    调用Wake_Up通知线程的所有者。Wake_Up将唤醒current->joinQueue上的所有线程
	Detach_Thread通知Reaper内核线程"收割"此线程。
	*/
