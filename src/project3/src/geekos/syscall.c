 ...
 * Returns: the global semaphore id
 函数首先检查请求创建的这个信号量的名字是否存在，如果存在，
 那么就把这个线程加入到这个信号量所注册的线程链表上；如果不存在，
 则分配内存给新的信号量，清空它的线程队列，把当前的这个线程加入到它的线程队列中，
 设置注册线程数量为1，初始化信号量的名字，值和信号量的ID，
 并把这个信号量添加到信号量链表上，最后返回信号量的ID
 */
 ...
 * Returns: 0 if successful, error code (< 0) if unsuccessful
 假设进程具有访问信号量的权限，该调用将阻塞，直到信号量计数为>= 0

 
 首先检查传入的信号量ID是否存在，如果存在，接着检查当前线程是否注册使用了这个信号量，
 如果这两项检查任意一项失败了，那么就返回-1。如果成功了，就把信号量的值减去1，
 ...
 *
 * Returns: 0 if successful, error code (< 0) if unsuccessful
 首先也是检查传入的信号量ID是否存在，如果存在，
 接着检查当前线程是否注册使用了这个信号量，如果这两项检查任意一项失败了，
 那么就返回-1。如果成功了，那就把信号量的值加上1，
 ...
 * Returns: 0 if successful, error code (< 0) if unsuccessful
 ...
 首先也是检查传入的信号量ID是否存在，如果存在，
 接着检查当前线程是否注册使用了这个信号量，如果这两项检查任意一项失败了，
 那么就返回-1。如果成功了，就把该线程从这个信号量的注册的线程数组中删除，
 并把注册的线程数量减去1。如果这个信号量的注册线程为0了，
 则把这个信号量从信号量链表中删除，并释放它的内存。
