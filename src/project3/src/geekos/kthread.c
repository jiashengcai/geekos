...
/*********************   ce123 Adds   *********************/  
/* By default the scheduling policy is Round Robin */ 
int g_schedulingPolicy = ROUND_ROBIN;  
...
static void Destroy_Thread(struct Kernel_Thread* kthread)
{
	...
	if (kthread->userContext != 0){
		if (kthread->userContext->memory != 0){			
			Free(kthread->userContext->memory);
		}
		Free(kthread->userContext);
	}
...
/* TODO("Start user thread"); */
	...
	kt = Create_Thread(USER_PRIVILEGE, false);
	if (kt != NULL) {
		Setup_User_Thread(kt, userContext);
		Make_Runnable_Atomic(kt);
	}
		return kt;
}
